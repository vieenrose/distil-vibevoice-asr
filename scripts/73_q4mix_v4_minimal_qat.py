#!/usr/bin/env python
"""q4mix-v4 QAT: MINIMAL-PERTURBATION silence-margin training.

WHY v4 (v2c/v3 post-mortem, 2026-07-23): three full-recipe variants fixed
silence perfectly but destroyed accuracy (v3 screen: zh 56->0%, en 12-23% vs
v1's 89/99). Mechanism: CE on 500 SELF-LABELED chunks x 440M trainable params
re-taught the teacher's en meeting errors (the v10b lesson, amplified 73x) and
the silence pressure eventually swamped 40 patience examples. The fix needs to
move ONE marginal decision (~+0.4 logits of EOS margin in silence contexts --
measured by the diagnostic); v1's accuracy is already the target, so v4 stops
trying to relearn transcription at all:
  * TRAIN ONLY the silence-class pool: silence-negatives (40% of steps, with
    recovery prefixes), patience composites (30%, full-CE -- wait through
    lead-ins then transcribe), quiet-speech positives (30%, full-CE) -- the
    NORMAL SELF-LABEL POOL IS GONE, so self-label poisoning cannot occur.
  * L2 ANCHOR to initialization (trust region): loss += l2_lambda *
    mean((w - w0)^2) over trainable latents -- broad drift is penalized,
    the narrow margin shift is cheap.
  * lr 1e-5 (10x down), 300 steps, save EVERY 25 steps: v3's curves show the
    silence objective converges by ~step 200; the dense ladder lets the gate
    pick the earliest checkpoint that flips silence while goldens stay ~v1.
  * CANARY (monitor-only): forced-CE on 8 held normal chunks every 25 steps;
    it TRAINS ON NOTHING -- if it rises >2x its step-0 value, drift is under
    way and later checkpoints are suspect.
...
def main() -> int:
    ...
    ap.add_argument("--silence-weight", type=float, default=2.0,
    ...
    ap.add_argument("--out", default="models/moss_q4mix_v4_qat")
    ap.add_argument("--l2-lambda", type=float, default=50.0,
                    help="trust-region anchor: loss += lambda*mean((w-w0)^2)")
    ap.add_argument("--canary-n", type=int, default=8)
    ...
    # trust-region anchor: snapshot of every trainable latent at init
    anchors = {id(m): m.lin.weight.detach().clone()
               for m in model.modules()
               if isinstance(m, QATLinear) and m.lin.weight.requires_grad}
    def l2_anchor():
        acc = 0.0
        n = 0
        for m in model.modules():
            if isinstance(m, QATLinear) and id(m) in anchors:
                d = m.lin.weight - anchors[id(m)]
                acc = acc + (d * d).sum()
                n += d.numel()
        return acc / max(1, n)
    model.train()
    ...
    # canary set: held normal chunks, MONITOR-ONLY (never trained)
    canary = pool[:args.canary_n]
    ...
    def canary_ce():
        ...
        for c in canary:
            ...
            logits = out.logits[0, n_prompt - 1:-1, :]
            targets_ = batch["input_ids"][0, n_prompt:]
            tot += F.cross_entropy(logits.float(), targets_).item()
            n += 1
        model.train()
        return tot / max(1, n)

    c0 = canary_ce()
    print(f"canary forced-CE at init: {c0:.4f} (drift alarm at {2*c0:.4f})",
          flush=True)

    sil_rng = random.Random(17)
    step = micro = 0
    running = {"s": 0.0, "p": 0.0, "q": 0.0}
    counts = {"s": 0, "p": 0, "q": 0}
    pat_pool = [r for r in quiet_pool if r.get("patience")]
    att_pool = [r for r in quiet_pool if not r.get("patience")]
    assert pat_pool and att_pool, "need both patience and quiet examples"
    t0 = time.time()
    while step < args.steps:
        ...
        if r < 0.40:
            ...
            kind, loss = "p", quiet_step(sil_rng.choice(pat_pool))
        else:
            kind, loss = "q", quiet_step(sil_rng.choice(att_pool))
        if loss is None:
            continue
        loss = loss + args.l2_lambda * l2_anchor()
        ...
        if micro % args.accum == 0:
            ...
            if step % args.save_every == 0 or step == args.steps:
                ...
                msg = " ".join(
                    ...
                    for k in ("s", "p", "q"))
                alarm = " DRIFT-ALARM" if cc > 2 * c0 else ""
                print(f"step {step:5d}: {msg} canary={cc:.4f}{alarm} "
